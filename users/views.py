...
async def create_user(user_data: CreateUserScheme, db_session: so.Session = Depends(get_session)):
    ...
    if result:
        if result.username == user_data.username:
            raise HTTPException(status_code=http_status.HTTP_409_CONFLICT, detail="Username already exists.")
        elif result.phone_number == user_data.phone_number:
            raise HTTPException(status_code=http_status.HTTP_409_CONFLICT, detail="Phone number already exists.")
        elif result.email_address == user_data.email_address:
            raise HTTPException(status_code=http_status.HTTP_409_CONFLICT, detail="Email address already exists.")
